...
Points in a ring should not be repeated (except first-last in case of GML, see :ref:`e103`). This error is reported when 2 *consecutive* points are at the same location.
...
It is the same if the vertices *b* and *c* are projected to the same location: a self-intersection is also returned.
...
To verify this, we must ensure that the distance between every point and a plane is less than a given *tolerance* (eg 1cm).
In the validator, this plane is fitted with least-square adjustment, and the distance between each of the points to the plane is calculated.
...
In practice it can be implemented with a triangulation of the polygon (any triangulation): the orientation of the normal of each triangle must not deviate more than a certain user-defined tolerance.
...
Also, 204 usually means that the vertices in the polygon are *very* close to each other (say 0.1mm), and thus it's easy to get a large deviation (say 80 degrees; the report contains the actual deviation).
...
When the polygon is used as a bounding surface of a shell, then the rings have to have a specified orientation (see :ref:`e307`).
...
Imagine you have a cube with 6 surfaces, if some surfaces have their normal pointing outwards, and some inwards, then this error might be returned (or :ref:`e405`, depending on the configuration).
...
If topology of the shell is correct and the shell is closed (thus no error 301/302/303/305), it is possible that the geometry introduces errors, eg intersections.
...
The interior of two or more shells intersect, these can be either the exterior shells with an interior shell or two or more interior shells.
...
The adjacency of IndoorGML cells in the primal and the dual are not consistent. Basically, if two 3-cells are adjacent in the primal, are they also in the dual (do they have a dual edge)? The :ref:`option_overlap_tol` influences this.
...
The input file contains empty geometric primitives, eg an OBJ where no surfaces are defined, or a ``CompositeSolid`` in a CityJSON file containing one Solid with no vertices and/or surfaces.
...
If none of the above is suitable, which means something really bad went wrong. If you see this error, `please report it <https://github.com/tudelft3d/val3dity/issues>`_.
