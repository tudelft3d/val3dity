...
def validate(file, planartol=0.01):
    str1 = VAL3DITYEXE + " --unittests --planarity_d2p " + str(planartol) + " " + SOLIDFOLDER + file +'.poly'
    # str1 = "../val3dity --unittests --planarity_d2p 0.01 ../data/poly/toofewpts.poly"
    # print str1
    op = subprocess.Popen(str1.split(' '), stdout=subprocess.PIPE)
    R = op.poll()
    if R:
        res = op.communicate()
