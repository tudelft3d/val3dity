...
@pytest.fixture(scope="module", params=["501.json", "501_1.json"])
def data_501(request, dir_geometry_specific):
    file_path = os.path.abspath(os.path.join(dir_geometry_specific, request.param))
    return file_path


@pytest.fixture(scope="module", params=["501.json"])
def data_501_overlap(request, dir_geometry_specific):
    file_path = os.path.abspath(os.path.join(dir_geometry_specific, request.param))
    return file_path


@pytest.fixture(scope="module", params=["502.json"])
def data_502(request, dir_geometry_specific):
    file_path = os.path.abspath(os.path.join(dir_geometry_specific, request.param))
    return file_path


@pytest.fixture(scope="module", params=["503.json"])
def data_503(request, dir_geometry_specific):
    file_path = os.path.abspath(os.path.join(dir_geometry_specific, request.param))
    return file_path


@pytest.fixture(scope="module", params=["601.city.json", "601_1.json"])
def data_601(request, dir_geometry_specific):
    file_path = os.path.abspath(os.path.join(dir_geometry_specific, request.param))
    return file_path


@pytest.fixture(scope="module", params=["601_1.json"])
