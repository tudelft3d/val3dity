...
# ------------------------------------------------------------------------ Data
@pytest.fixture(scope="module", params=["building.fg.json", "cubes.fg.json"])
def data_jsonfg_valid(request, dir_jsonfg):
    file_path = os.path.abspath(os.path.join(dir_jsonfg, request.param))
    return [file_path]


@pytest.fixture(scope="module", params=["cubes2.fg.json"])
def data_jsonfg_one_geojson(request, dir_jsonfg):
    file_path = os.path.abspath(os.path.join(dir_jsonfg, request.param))
    return [file_path]


@pytest.fixture(scope="module", params=["e103.fg.json"])
def data_jsonfg_invalid(request, dir_jsonfg):
    file_path = os.path.abspath(os.path.join(dir_jsonfg, request.param))
    return [file_path]


@pytest.fixture(scope="module", params=["geojson.json"])
