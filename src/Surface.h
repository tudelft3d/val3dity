...
private:
  ...
  bool construct_ct(const std::vector< std::vector<int> >& pgnids, const std::vector<CGALPolygon>& lsRings, std::vector<int*>& oneface, int faceNum, const CgalPolyhedron::Plane_3 &plane);
  bool validate_polygon(std::vector<CGALPolygon> &lsRings, std::string polygonid);
  bool validate_projected_ring(CGALPolygon &pgn, std::string id);
