...
bool Polygon::validate(double tol_planarity_d2p, double tol_planarity_normals, double tol_overlap)
{
  if (this->is_valid() == 0)
    return false;
...
Primitive3D Polygon::get_type() 
{
  return POLYGON;
}

int Polygon::is_valid() {
...
std::string Polygon::get_off_representation()
{
  return _surface->get_off_representation();
}


void Polygon::get_min_bbox(double& x, double& y)
{
  _surface->get_min_bbox(x, y);
}


void Polygon::translate_vertices()
{
  _surface->translate_vertices();
}

bool Polygon::is_empty() {
  return _surface->is_empty();
}


json Polygon::get_report_json()
{
  json j;
  bool isValid = true;
  j["type"] = "Polygon";
...
int Polygon::num_faces() 
{
  return _surface->number_faces();
}

int Polygon::num_vertices() 
{
  return _surface->number_vertices();
}


bool Polygon::set_surface(Surface* s) 
{
  _surface = s;
  return true;
}

std::set<int> Polygon::get_unique_error_codes() {
